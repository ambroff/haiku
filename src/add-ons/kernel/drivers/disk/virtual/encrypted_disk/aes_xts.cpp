...
void
aes_xts_reinit(struct aes_xts_ctx* ctx, uint8_t* iv)
{
	uint64_t blocknum;
	unsigned int i;
	...
	memset(ctx->tweak + AES_XTS_IVSIZE, 0, AES_XTS_IVSIZE);
...
void
aes_xts_encrypt(struct aes_xts_ctx* ctx, uint8_t* data)
{
...
void
aes_xts_decrypt(struct aes_xts_ctx* ctx, uint8_t* data)
{
...
int
aes_xts_setkey(struct aes_xts_ctx* ctx, uint8_t* key, int len)
{
	if (len != 32 && len != 64)
		return -1;
...
{
	memset(ctx, 0, sizeof(struct aes_xts_ctx));
}
